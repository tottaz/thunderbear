...
if ( is_tax( 'download_category' ) || is_tax( 'download_tag' ) ) {
	...
	if ( 'download_category' === $download_term->taxonomy ) {
		$term_type = _x( 'Category', 'download category archive page title', 'thunderbear' ) . ': ';
	} elseif ( 'download_tag' === $download_term->taxonomy ) {
		$term_type = _x( 'Tag', 'download tag archive page title', 'thunderbear' ) . ': ';
	}
	?>
...
if ( have_posts() ) : $i = 1;
			...
			<div id="edd_download_pagination" class="store-pagination navigation">
				<?php
					...
					echo paginate_links( array(
						...
						'prev_text'	=> __( '&laquo; Previous', 'thunderbear' ),
						'next_text'	=> __( 'Next  &raquo;', 'thunderbear' ),
					) );
				?>
			</div>
		<?php endif; ?>
	...
	<section class="store-404 not-found">
		<header class="page-header">
			<h1 class="page-title"><?php _e( 'Oops! There\'s nothing to see here.', 'thunderbear' ); ?></h1>
		</header><!-- .page-header -->

		<div class="page-content">
			<p><?php _e( 'It looks like nothing was found at this location. Maybe try one of the links below or a search?', 'thunderbear' ); ?></p>

			<?php get_search_form(); ?>

			<?php the_widget( 'WP_Widget_Recent_Posts' ); ?>

			<?php if ( thunderbear_categorized_blog() ) : // Only show the widget if site has multiple categories. ?>
			<div class="widget widget_categories">
				<h2 class="widget-title"><?php _e( 'Most Used Categories', 'thunderbear' ); ?></h2>
				<ul>
				<?php
